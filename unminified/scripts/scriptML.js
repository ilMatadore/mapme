...
function readBlob() {
    ...
    var reader = new FileReader(); /* generates table based on file data */
        ...
        if (evt.target.readyState == FileReader.DONE) {
            ...
            for (var i = 0; i < refPts.length; i++) {
                if (refPts[i].length > 0) {
                    ...
                    cellgoMap.textContent = ""; /*creates map icon button */
                    ...
                    goMap.onclick = mapeame; /* map each location function */
                    function mapeame(evt) {
                        ...
                        if (evt.target.parentElement.nextElementSibling == null) {
                            ...
                            function geocodingCallback(error, result) {
                                ...
                                longitude.textContent = result.results[0].locations[0].displayLatLng.lng; /* color marker var customIcon=L.mapquest.icons.marker({primaryColor: '#ffa500'}); L.marker([latitude.textContent, longitude.textContent] ,{icon: customIcon}).addTo(map); */
                            }
                        } else {
                            L.mapquest.geocoding().geocode(inputToMap);
                            L.mapquest.geocoding().geocode(inputToMap, geocodingCallback);
                            ...
                            function geocodingCallback(error, result) {
                                ...
                                evt.target.parentElement.parentElement.childNodes[8].textContent = result.results[0].locations[0].displayLatLng.lng; /* color marker var customIcon=L.mapquest.icons.marker({primaryColor: '#ffa500'}); L.marker([evt.target.parentElement.parentElement.childNodes[7].textContent, evt.target.parentElement.parentElement.childNodes[8].textContent] ,{icon: customIcon}).addTo(map); */
                            };
                        };
                    };
                };
            ...
            if (loadOpts.childNodes.length == 5) {
...
document.querySelector('#submit').addEventListener('click', function(evt) { /*removes rows when loading a new file*/
