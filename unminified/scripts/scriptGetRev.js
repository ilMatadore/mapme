function readBlobRev() {
    ...
    function createMap(error, response) {
        ...
        map = L.mapquest.map('map', {
            ...
            zoom: 12
        }); /*var customIcon=L.mapquest.icons.marker({primaryColor: '#FF9531'});L.marker(latLng,{icon: customIcon}).addTo(map);*/
    };
    var readerRev = new FileReader(); /* generates table based on file data */
                ...
                if (refPtsRev[i].length > 0) {
                    ...
                    function mapeameRev(evt) {
                        ...
                        if (evt.target.parentElement.nextElementSibling == null) {
                            ...
                            evt.target.parentElement.parentElement.appendChild(countryRev); /*var reverseMap=revToMap[evt.target.id.split('')[evt.target.id.split('').length-1]-1]; */
